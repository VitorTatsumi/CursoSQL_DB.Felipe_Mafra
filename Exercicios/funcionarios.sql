...
--O gestor de marketing pediu a lista das funcionárias que trabalhem no departamento de filmes ou no departamento lar. Ele necessita enviar um email para as colaboradoras desses dois setores.
--Antes de tudo é necessário utilizar a função COUNT e GROUP BY para fazer a verificação de quantos elementos com o valor especificado cada coluna tem, e assim é possível determinar a melhor rota de codificação para gerar menos processamento.

SELECT COUNT (*) AS QTD, SEXO 
FROM funcionarios 
GROUP BY SEXO;

SELECT COUNT (*) AS QTD, DEPARTAMENTO 
FROM funcionarios 
GROUP BY DEPARTAMENTO
ORDER BY 1;
...
--Traga os funcionários do sexo masculino ou os funcionários que trabalham no Jardim
